...
class Firmware(object):
	...
	def load_from_file(self, filename):
		...
		with open(filename, "rb") as firmware_file:
			...
			self._logger.info("%i bytes remain", (filesize - firmware_file.tell()))
	...
	def generate_header_file(self, filename):
		with open(filename, "wb") as header_file:
			...
			if(res is not None):
				...
				#since it might be a swapped in section, so address is result + 0x5000
...
def get_address_map(filename):
	regex = re.compile(r"^\s*(?P<address>[0-9a-fA-F]+)\s+(\S+\s+)*(?P<label>_\S*):$")
	address_map = {}
